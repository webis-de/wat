...
public class WatServletServer
{
	...
	public static void main(final String[] args) throws Exception
	{
		...
		final ServletHolder annotationServlet = new ServletHolder("wat", WatServlet.class);
		context.addServlet(annotationServlet, SERVLET_PATH + "/*");

		final ServletHolder resourcesServlet = new ServletHolder("static-embedded", DefaultServlet.class);
	...
	private static ResourceCollection createBaseResource() throws Exception
	{
		final String embeddedBase =
				WatServletServer.class.
				getClassLoader()
				.getResource(EMBEDDED_RESOURCES_DIR)
				.toExternalForm();
		
		String[] bases = new String[] { embeddedBase };

		final File localBaseFile = new File(new File(System.getProperty("user.dir")), LOCAL_RESOURCES_DIR);
