...
public class ComponentsFactory {
	...
	public static ComponentsFactory load(final String[] componentEntries, final File projectDirectory,
			final File tasksDirectory, final boolean anonymizeAnnotatorsForCuration) throws IllegalArgumentException {
		...
		for (final String componentEntry : componentEntries) {
			...
			if (keyValue.length != 2) {
				throw new IllegalArgumentException("No '<component-name>=<component-class>' pair: " + componentEntry);
			}
			componentNames.add(keyValue[0]);
			classStrings.add(keyValue[1]);
		}

		return new ComponentsFactory(componentNames, classStrings, projectDirectory, tasksDirectory,
				anonymizeAnnotatorsForCuration);
	}

	public ComponentsFactory(final List<String> componentNames, final List<String> classStrings,
			final File projectDirectory, final File tasksDirectory, final boolean anonymizeAnnotatorsForCuration)
		...
		if (componentNames.size() != classStrings.size()) {
			throw new IllegalArgumentException("Unequal number of component names and class strings: "
	...
	private Constructor<? extends Component> getConstructor(final String classString) throws IllegalArgumentException {
		try {
			...
			if (Component.class.isAssignableFrom(componentClassGeneric)) {
				...
				final Class<? extends Component> componentClass = (Class<? extends Component>) componentClassGeneric;
				return componentClass.getConstructor(String.class, File.class, File.class, Map.class);
			} else {
				throw new IllegalArgumentException(
						"Class " + classString + " does not implement " + Component.class.getName());
			}
		} catch (final ClassNotFoundException e) {
			throw new IllegalArgumentException(e);
		} catch (final NoSuchMethodException e) {
			throw new IllegalArgumentException(
					"Class " + classString + " does not implement the required constructor for components");
		}
	}

	public List<Component> loadComponents(final String taskName) throws IOException {
		final File taskDirectory = new File(this.tasksDirectory, taskName);
		final Map<String, RecordsFile> recordFilesByAnnotator = this.loadRecordFilesByAnnotator(taskDirectory);
		...
		for (int c = 0; c < size; ++c) {
			...
			final Constructor<? extends Component> constructor = this.constructors.get(c);

			try {
				final Component component = constructor.newInstance(componentName, this.projectDirectory, taskDirectory,
						recordFilesByAnnotator);
				components.add(component);
			} catch (final InvocationTargetException | InstantiationException | IllegalAccessException e) {
				throw new IllegalArgumentException("Could not construct component from " + constructor, e);
			}
		}
	...
	private Map<String, RecordsFile> loadRecordFilesByAnnotator(final File taskDirectory) throws IOException {
		final Map<String, RecordsFile> recordFilesByAnnotator = new HashMap<>();
		if (taskDirectory.isDirectory()) {
			for (final RecordsFile recordsFile : RecordsFile.readAllInDirectory(taskDirectory)) {
				recordFilesByAnnotator.put(recordsFile.getAnnotator(), recordsFile);
			}
		}

		if (this.anonymizeAnnotatorsForCuration) {
			final List<String> annotators = new ArrayList<>(recordFilesByAnnotator.keySet());
			// implementing a shuffling that this consistent between server
			// reloads
			Collections.sort(annotators);
			Collections.shuffle(annotators, new Random(taskDirectory.getName().hashCode()));
