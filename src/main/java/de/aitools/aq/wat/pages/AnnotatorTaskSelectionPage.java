...
{
	private static final DateFormat DATE_FORMAT = AnnotatorTaskSelectionPage.compileDateFormat("yyyy-MM-dd HH:mm");
	...
	public AnnotatorTaskSelectionPage(final String projectName) throws NullPointerException
	{
		super(projectName);
	}
	...
	protected void printInFileScripts(final PrintWriter output, final Annotator annotator, final TaskState state)
	{
		...
		output.append("var first = document.getElementById(\"" + FIRST_UNCOMPLETED_TASK_ID + "\");");
	...
	protected void printMain(final PrintWriter output, final Annotator annotator, final TaskState state)
	{
		...
		output.append("  Choose a task below in order to work on it.").append(" All progress is saved automatically.")
	...
	private int printProgressPanel(final PrintWriter output, final Annotator annotator)
	{
	...
	private void printTaskSelectionTable(
			...
			final int numTasksCompleted)
	{
		...
		for (final Entry<Task, TaskState> entry : annotator)
		{
			...
			final String lastUpdateString = lastUpdate == null ? "-" : DATE_FORMAT.format(lastUpdate);
			...
			output.append("      <td>").append("<button type=\"submit\" class=\"btn btn-default\" name=\"")
					.append(WatServlet.REQUEST_PARAMETER_TASK)
					.append("\" value=\"")
					.append(name)
					.append("\">")
					.append(name)
			...
			output.append("      <td>")
					.append(state.getProgress())
					.append("</td>\n");

			output.append("      <td>")
					.append(lastUpdateString)
					.append("</td>\n");
