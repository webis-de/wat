...
public abstract class AnnotatorPage extends Page
{
	...
	public AnnotatorPage(final String projectName) throws NullPointerException
	{
		super(projectName);
	}

	public AnnotatorPage(final String projectName, final List<String> cssIncludes, final List<String> jsIncludes)
			throws NullPointerException
	{
		super(projectName, cssIncludes, jsIncludes);
	}

	public void print(final PrintWriter output, final Annotator annotator, final TaskState state,
	...
	protected abstract void printInFileScripts(
			...
			final boolean isAdminLogin)
	{
		output.append("<input type=\"hidden\" name=\"")
			.append(WatServlet.REQUEST_PARAMETER_TIME_ZONE_OFFSET)
			.append("\"/>");
		...
		if (annotator != null)
		{
			...
			output.append("<input id=\"task-internal-field-password\" type=\"hidden\" name=\"")
					.append(WatServlet.REQUEST_PARAMETER_PASSWORD).append("\" value=\"").append(annotator.getPassword())
					.append("\" />");
		...
		if (state != null)
		{
			output.append("<input id=\"task-internal-field-task\" type=\"hidden\" name=\"")
					.append(WatServlet.REQUEST_PARAMETER_TASK).append("\" value=\"").append(state.getTask().getName())
					.append("\" />");
		}
	}
			...
			final boolean isAdminLogin)
	{
		...
		output.append("    <a class=\"navbar-brand\" href=\"#\">").append(this.getProjectName()).append("</a>\n");
		...
		if (annotator != null)
		{
			...
			if (state != null)
			{
				output.append("&commat;").append(state.getTask().getName().replaceAll("\\s+", "&nbsp;"));
			}
	...
	protected abstract void printMain(final PrintWriter output, final Annotator annotator, final TaskState state);

	private void printBottomNavbar(final PrintWriter output, final Annotator annotator, final TaskState state)
	{
		if (annotator != null)
		{
			output.append("<nav class=\"navbar navbar-default\">\n");
	...
	private void printNavbarButtons(final PrintWriter output, final boolean logout, final boolean taskSelection)
	{
		output.append("    <div class=\"btn-group\">\n");

		if (logout)
		{
			output.append("      <button type=\"button\" class=\"").append(BUTTON_CLASS_LOGOUT)
					.append(" btn btn-default navbar-btn\">logout</button>\n");
		}

		if (taskSelection)
		{
			output.append("      <button type=\"button\" class=\"").append(BUTTON_CLASS_TASK_SELECTION)
