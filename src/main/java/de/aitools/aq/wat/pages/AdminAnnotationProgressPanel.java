...
public class AdminAnnotationProgressPanel extends Panel
{
	...
	protected void printPanelContent(final PrintWriter output)
	{
		...
		output.println("When logged in using the buttons above, all actions"
		    + "performed will be ignored when " + "calculating the time spent.");
	}
	...
	protected void printAnnotatorProgress(final PrintWriter output, final int numTasks, final int numTasksCompleted, final Annotator annotator)
	{
		...
		for (final Entry<Task, TaskState> task : annotator)
		{
			...
			String[]t = temp.split("/");
			doneSegments = doneSegments + Integer.parseInt(t[0]);
			todoSegments = todoSegments + Integer.parseInt(t[1]);
		}
		
		output.print("<td class='right'>");
		output.print(doneSegments + "/" + todoSegments);
		output.print("</td>");
		
		
		output.print("<td class='right'>");
		double segRatio = ((double) doneSegments) / todoSegments;
		output.print("" + String.valueOf(Math.round(segRatio * 100)) + "%");
		output.print("</td>");
		
	}

	protected void printAnnotatorWorkTime(final PrintWriter output, final int workTimeInSeconds)
	{
	...
	protected void printLoginButton(final PrintWriter output, final Annotator annotator)
	{
		output.print("<td>");
