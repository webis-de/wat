...
public class TextBox extends Component {
	...
	public TextBox(final String name, final File projectDirectory, final File taskDirectory,
	...
	public void printHtmlInPanel(final PrintWriter output) {
		output.append("<textarea class=\"form-control\" rows=\"5\" id=\"").append(this.getName()).append('-')
				.append(KEY).append("\" onblur=\"javascript:update('").append(this.getName()).append("', '").append(KEY)
				.append("', $('#").append(this.getName()).append('-').append(KEY).append("').val())\"></textarea>");
		...
		for (final String text : this.unit.getLabels()) {
			for (final String annotator : this.unit.getAnnotators(text)) {
				...
				output.append(":");
				output.append("<textarea class=\"form-control\" rows=\"5\"").append("disabled=\"disabled\">");
