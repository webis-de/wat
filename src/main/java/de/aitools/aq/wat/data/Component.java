...
public abstract class Component implements Iterable<Component.Unit> {
			...
			throws IOException
	{
		...
		{
			throw new NullPointerException("The Javascript value function name must not be null");
		}
		...
		final File projectConfigFile = new File(projectDirectory, this.getName() + PropertiesFiles.SUFFIX);
		final File taskConfigFile = new File(taskDirectory, this.getName() + PropertiesFiles.SUFFIX);
		final Properties config = PropertiesFiles.load(PropertiesFiles.load(projectConfigFile), taskConfigFile);
	...
	{
		this.setRequired(Boolean.parseBoolean(config.getProperty(CONFIG_FIELD_REQUIRED, "true")));
		this.setUseMajorityAnnotation(Boolean.parseBoolean(config.getProperty(CONFIG_FIELD_USE_MAJORITY_ANNOTATION, "true")));
		this.setTitle(config.getProperty(CONFIG_FIELD_TITLE));
		this.setDescription(config.getProperty(CONFIG_FIELD_DESCRIPTION));
		this.addCssIncludes(config.getProperty(CONFIG_FIELD_CSS_INCLUDES, "").split("\\s+"));
		this.addJsIncludes(config.getProperty(CONFIG_FIELD_JS_INCLUDES, "").split("\\s+"));
	}

	protected void addCurationInfo(final Map<String, RecordsFile> recordFilesByAnnotator) throws IOException {
		for (final Entry<String, RecordsFile> entry : recordFilesByAnnotator.entrySet()) {
			final String annotator = entry.getKey();
			final RecordsFile recordsFile = entry.getValue();
			final Stream<Record> records = StreamSupport.stream(recordsFile.spliterator(), false)
	...
	protected void addCurationInfo(final String annotator, final Stream<Record> records) throws IOException {
		final Map<String, String> annotations = new HashMap<>();
		records.forEachOrdered(record -> annotations.put(record.getKey(), record.getValue()));
	...
	protected void addCssIncludes(final String... cssIncludes) {
		for (final String cssInclude : cssIncludes) {
			...
			if (!cssInclude.isEmpty()) {
				this.cssIncludes.add(cssInclude);
			}
		}
	}
	...
	protected void addJsIncludes(final String... jsIncludes) {
		for (final String jsInclude : jsIncludes) {
			...
			if (!jsInclude.isEmpty()) {
				this.jsIncludes.add(jsInclude);
			}
		}
	}
	...
	public static class Unit {
		...
		private void checkMajorityAnnotation() {
			if (this.maxAnnotationsForLabel < ((double) this.numAnnotations) / 2.0) {
				this.majorityAnnotation = null;
			}
		}
